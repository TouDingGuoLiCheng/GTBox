...
def preprocess_ocr_line(text: str) -> str:
    ...
    while prev != value:
        ...
        value = re.sub(r"\s+S?VIP\s*$", "", value, flags=re.IGNORECASE)
        # 空格 + DTS:X / DTS X / DTS-X / DTS·X（含全角冒号/中点；杜比音效角标）
        # 例：「卡农 (Canon) DTS:X」「演奏:dylanf VIP DTS X」→ 去掉 DTS X 段，保留前文
        value = re.sub(
            r"\s+DTS\s*[:：\-‐\-·・]?\s*X(?=\s|$)",
            " ",
            value,
...
def strip_ui_noise(text: str) -> str:
    ...
    # 杜比 DTS:X / DTS X / DTS-X / DTS·X 标签（线内出现也直接删除）
    value = re.sub(r"\bDTS\s*[:：\-‐·・]?\s*X\b", "", value, flags=re.IGNORECASE)
...
def resolve_images_from_args(args: argparse.Namespace, base_dir: Path) -> tuple[list[Path], Path | None]:
    """返回待识别图片列表；第二项为目录模式下的 input_dir（显式图片列表时为 None）。"""
    raw_images = getattr(args, "images", None)
    if raw_images and str(raw_images).strip():
        paths: list[Path] = []
        for part in str(raw_images).split("|"):
            part = part.strip()
            if not part:
                continue
            p = Path(part)
...
def export_regions_for_ui(
    ...
    每个 region 都附带原图宽高（imageWidth/imageHeight），前端据此换算显示坐标，
    避免预览缩略图被下采样后框位置错位。
    """
    out: list[dict[str, Any]] = []

    def _attach_dims(item: dict[str, Any]) -> dict[str, Any]:
        if image_width > 0:
...
def run(args: argparse.Namespace) -> int:
    ...
    images, input_dir = resolve_images_from_args(args, base_dir)
    ...
    if not images:
        hint = (
            f"未在目录找到截图: {input_dir}"
            if input_dir is not None
            else "未找到有效截图文件（--images 路径无效或不存在）"
        )
        print(hint)
        return 1

    print(
        f"[OCR] 参数: device={args.device} merge={args.merge} "
    ...
    for idx, image_path in enumerate(images, start=1):
        ...
        if regions_dir is not None:
            ...
            print(f"[OCR] [{image_path.name}] 已写入 regions 缓存", flush=True)

        print(
            f"[OCR] [{image_path.name}] 框={len(boxes)} 行={len(rows)} "
            f"配对={len(pairs)} 未配对={len(unpaired)} 歌曲={len(tracks)} review={len(reviews)}",
            flush=True,
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--images",
        default=None,
        help="指定截图文件列表（| 分隔的绝对/相对路径），设置后不再扫描 --input 目录",
    )
