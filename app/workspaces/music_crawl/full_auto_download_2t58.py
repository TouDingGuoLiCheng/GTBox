...
def _ensure_quark_file_checked(page, timeout_ms: int = 1200) -> bool:
    """夸克分享页可能已自动勾选文件 checkbox。这里逐个判断：
    - 已勾选：跳过，避免再次点击把状态切回未选。
    - 未勾选：尝试点击勾上。
    返回是否找到至少一个 checkbox（不论原本是否已选）。
    """
    selectors = [
    ...
    for sel in selectors:
        ...
        for i in range(min(count, 5)):
            box = loc.nth(i)
            try:
                if box.is_checked(timeout=timeout_ms):
                    continue
            except Exception:
                # is_checked 不可用（非标准 checkbox）就保守地不点
                continue
            try:
...
def download_from_website_http(
    ...
    for idx, row in enumerate(rows, start=1):
        # 2t58 触发了「今日下载次数已达上限」之后，剩余歌曲再请求也只会返回 alert 页，
        # 没必要继续浪费时间——把剩下的标成 skip 并直接跳出，让前端弹窗提示用户更新 Cookie。
        if site_limited:
            row.status = "skip_site_limited"
            row.note = "site_daily_download_limit"
            continue
        ...
        if not downloaded:
            ...
            print(f"  -> failed: {row.note}")
            # 触发站点级限制后立刻终止整批；前端会捕获该日志弹窗提示更新 Cookie。
            if last_error == "site_daily_download_limit":
                site_limited = True
                print(
                    "[site-limit] 2t58 今日下载次数已达上限，剩余歌曲已跳过。"
                    "请在浏览器完成口令验证后，更新 Cookie 再重试失败项。"
                )
...
QUARK_LOGIN_COOKIE_NAMES = {"__pus", "__puus", "__t", "__tk"}
QUARK_LOGIN_WAIT_TIMEOUT_SEC = 600  # 10 分钟
QUARK_LOGIN_POLL_SEC = 3


def _is_quark_logged_in(context: Any, page: Any) -> bool:
    """通过 cookie 与可选 DOM 信号判断当前 context 是否已登录夸克网盘。"""
    try:
        cookies = context.cookies("https://pan.quark.cn")
        names = {c.get("name") for c in cookies}
        if QUARK_LOGIN_COOKIE_NAMES & names:
            return True
    except Exception:
        pass
    # DOM 兜底：当前页若在 pan.quark.cn 且没被踢到 accounts.quark.cn，认为已登录
    try:
        cur = (page.url or "").lower()
        if cur.startswith("https://pan.quark.cn") and "accounts.quark.cn" not in cur:
            if page.locator("text=我的网盘").count() > 0:
                return True
    except Exception:
        pass
    return False
...
def _wait_for_quark_login(
    context: Any,
    page: Any,
    timeout_ms: int,
    timeout_sec: int = QUARK_LOGIN_WAIT_TIMEOUT_SEC,
    poll_sec: int = QUARK_LOGIN_POLL_SEC,
) -> bool:
    """暂停下载循环，导航到夸克首页等用户完成登录，轮询直到登录成功或超时。"""
    print("[login] 检测到未登录夸克网盘，已暂停后续下载。")
    print("[login] 请在已打开的浏览器中完成登录（最长等待 "
          f"{timeout_sec} 秒），登录成功后会自动继续。")
    # 跳到登录入口，确保用户能看到登录界面
    _goto_with_fallback(page, "https://pan.quark.cn/", timeout_ms=timeout_ms)
    start = time.time()
    last_log = 0.0
    while time.time() - start < timeout_sec:
        if _is_quark_logged_in(context, page):
            elapsed = int(time.time() - start)
            print(f"[login] 检测到登录成功（耗时 {elapsed}s），继续下载…")
            return True
        now = time.time()
        if now - last_log > 30:
            remain = int(timeout_sec - (now - start))
            print(f"[login] 仍在等待登录…剩余 {remain}s")
            last_log = now
        time.sleep(poll_sec)
    print("[login] 等待登录超时，剩余夸克下载将被跳过。")
    return False


def _ensure_quark_logged_in(context: Any, page: Any, timeout_ms: int) -> bool:
    """每次 goto 新分享链接前调用：未登录则暂停并等待登录完成。"""
    if _is_quark_logged_in(context, page):
        return True
    return _wait_for_quark_login(context, page, timeout_ms=timeout_ms)
...
def download_from_quark_rows(
    ...
    with sync_playwright() as p:
        ...
        # 进入循环前先做一次登录探测。manual_login_once 仅作为「主动触发首登」的信号，
        # 实际是否暂停由 _is_quark_logged_in 自动决定，无需用户在终端按回车。
        if manual_login_once or not _is_quark_logged_in(context, page):
            login_ok = _wait_for_quark_login(context, page, timeout_ms=timeout_ms)
            if not login_ok:
                for row in rows:
                    if row.quark_url and not row.status:
                        row.status = "skip_login_required"
                        row.note = row.note or "未在等待时间内完成夸克登录"
                # 仍走到清理逻辑，但不再 goto 任何链接
        ...
        for idx, row in enumerate(rows, start=1):
            ...
            # 每首歌 goto 前再确认一次：cookie 失效 / 用户主动登出时暂停
            if not _ensure_quark_logged_in(context, page, timeout_ms=timeout_ms):
                if not login_lost_logged:
                    print("[login] 登录超时未恢复，剩余歌曲跳过。")
                    login_lost_logged = True
                row.status = "skip_login_required"
                row.note = "未在等待时间内完成夸克登录"
                continue
            ...
            # 夸克分享页打开后，文件可能已被夸克自动勾选。无脑点 checkbox 反而会把已选的取消，
            # 导致后续点不到下载按钮。这里改成：先看 is_checked，只有未勾选时才点。
            _ensure_quark_file_checked(page, timeout_ms=1200)
